...
def sync_metrics_update(context):
    ...
    if reps is not None and reps_per_set > 0 and target_sets > 0:
        sets_completed = min(reps // reps_per_set, target_sets)  # cap at target
        current_set_reps = reps % reps_per_set if sets_completed < target_sets else reps_per_set
        workout_completed = sets_completed >= target_sets
        if workout_completed:
            current_set_reps = reps_per_set                  # show full last set
        else:
